...
public class CuentaAhorros extends CuentaBancaria {
    ...
    CuentaAhorros(ArrayList<Prestamo> prestamos){
        super(prestamos);
    }
    
    public void lanzarCuentas(){

        //Lanzamos los prestamos como hebras 
        for(int i=0; i<prestamo.size(); i++){
         ...
         while(!prestamo.isEmpty()){
             ...
             System.out.println("[P. ESTUDIOS PENDIENTES] en el año " + anio);
            ...
            for(int i = prestamo.size()-1; i >= 0; i--){
                if(prestamo.get(i).getAnios() == 0){
                    System.out.println("[P. ESTUDIOS] Su prestamo con id " + prestamo.get(i).obtenerID() + " y cantidad " + prestamo.get(i).getCantidad() + " ha vencido su plazo." );
                    prestamo.remove(i);
                }
            }
        }
